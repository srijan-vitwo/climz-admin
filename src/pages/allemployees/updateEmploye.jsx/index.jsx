...
const EmployeeUpdate = ({ details, fromValue, userData, msg, setMsg }) => {
    let explodedEmpCode = userData?.data?.emp_code ? userData?.data?.emp_code.match(/([A-Za-z]+)(\d+)/) ?? [] : [];
    let navigate = useNavigate();
    const toast = useToast()
    const token = localStorage.getItem("token");
    const FormTitles = ["EmpDetails", "Personal", "BankDetails", "KycDetails", "Document", "LastCompany"];
    ...
    const PageDisplay = () => {
        if (page === 0) {
            return <EmpDetails formData={formData} setFormData={setFormData} fromValue={fromValue} costCenter={costCenter} empCode={empCode} />;
        } else if (page === 1) {
            return <Personal formData={formData} setFormData={setFormData} fromValue={fromValue} />;
        } else if (page === 2) {
            return <BankDetails formData={formData} setFormData={setFormData} fromValue={fromValue} />;
        }
        else if (page === 3) {
            return <KycDetails formData={formData} setFormData={setFormData} fromValue={fromValue} />;
        }
        else if (page === 4) {
            return <Document formData={formData} setFormData={setFormData} fromValue={fromValue} />;
        }
        else {
            return <LastCompany formData={formData} setFormData={setFormData} fromValue={fromValue} />;
        }
    };
    ...
    const fromDataAdd = async () => {
        ...
        try {
            setIsLoading(true)
            const response = await fetch(`${process.env.REACT_APP_API_URL}/employee-update`, {
                method: "POST",
                body: fromValues,
                headers: {
                    Authorization: `Bearer ${token}`,
    ...
    return (
        ...
        <Box className="form">
            ...
            <Box className="form-container" p='15px 0px' bg='none' boxShadow='none'>
