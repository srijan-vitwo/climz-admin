...
const LocalLimits = () => {
  ...
  useEffect(() => {
    const travelLimit = async () => {
      try {
        ...
        const response2 = await fetch(`${process.env.REACT_APP_API_URL}/travel-local`, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
          },
        ...
        if (response1.ok && response2.ok) {
          ...
          setTravelLimit(tableInputData.data.local_limit);
          setEmpGread(tableColumn.data.grade_values);
          buildMyInputObj(tableInputData.data.local_limit);
          setLoader(false)
        } else {
          navigate('/login')
        }
  ...
  const travelLimitPost = async (e) => {
    ...
    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/travel-local-post`,
      {
        method: "POST",
        body: formData,
        headers: {
  ...
  return (
    <Box>
      ...
      <Box bg='white' borderRadius='15px' mb='30px'>
        <Heading mb='15px' color='claimzTextBlueColor'>List of all Local Limits</Heading>
        {loader ?
