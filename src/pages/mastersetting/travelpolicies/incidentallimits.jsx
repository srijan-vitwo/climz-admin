...
import { Box, Progress, Button, Input, Heading, Table, Tbody, Td, Th, Thead, Tr, useToast, Image } from "@chakra-ui/react";
import Loader from "../../../assets/images/loader.gif"

const IncidentalLimits = () => {
  ...
  useEffect(() => {
    const travelLimit = async () => {
      try {
        setLoader(true)
        const response1 = await fetch(`${process.env.REACT_APP_API_URL}/tier`, {
          method: "GET",
          headers: {
        ...
        const response2 = await fetch(`${process.env.REACT_APP_API_URL}/travel-incidental`, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
          },
        ...
        const response3 = await fetch(`${process.env.REACT_APP_API_URL}/emp-grade`, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
          },
        ...
        if (response1.ok && response2.ok) {
          ...
          setTravelLimit(tableInputData.data.incidental_limit);
          setEmpGread(tableColumn.data.grade_values);
          buildMyInputObj(tableInputData.data.incidental_limit);
          setLoader(false)
        } else {
          navigate('/login')
        }
  ...
  const travelLimitPost = async (e) => {
    ...
    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/travel-incidental-post`,
      {
        method: "POST",
        body: formData,
        headers: {
  ...
  return (
    <Box>
      ...
      <Box bg='white' borderRadius='15px' mb='30px'>
        <Heading mb='15px' color='claimzTextBlueColor'>List of all Incidental Limits</Heading>
        {loader ?
          ...
          <form onSubmit={travelLimitPost}>
            <Box display='flex' flexDirection='column' alignItems='flex-end'>
              <Box w='100%' overflowY='auto' height='calc(100vh - 345px)'>
                <Table className="reportsTable" variant="striped" >
                  ...
                  >
                    <Tr>
                      <Th color='white' p='15px 0px' textAlign='center' fontSize='1.4rem'></Th>
                      {travelMaster?.map((data, index) => {
                        return <Th key={index} color='white' p='15px 0px' textAlign='center' fontSize='1.4rem'>{data.type}</Th>
                      })}
                    </Tr>
                  </Thead>
                  <Tbody>
                    {empGread?.map((value, index) => {
                      return (<Tr key={index} >
                        <Td p='15px 0px' textAlign='center' px='15px' fontSize='1.4rem' fontWeight='500'>{value.grade_value}</Td>
                        {travelMaster?.map((data, index) => {
                          let inputKey = `c-${value.id}-${data.id}`;
                          return <Td key={index} p='15px 0px' textAlign='center' px='15px' fontSize='1.4rem' fontWeight='500'>
                            <Input type="number" onChange={e => changeMyInputObj(inputKey, e.target.value)} value={empTravelLimitInput[inputKey]} />
                          </Td>
                        })}
                      </Tr>)
                    })}
...
export default IncidentalLimits;
