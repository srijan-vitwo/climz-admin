  ...
  const ActionTemplate = (rowData) => {
    ...
    const tierUpdate = async (e) => {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/tier-update`,
        {
          method: "POST",
          body: formData,
          headers: {
          ...
          <DrawerContent maxW='50% !important' bgGradient='linear(180deg, #DCF9FF 0%, #FFFFFF 100%)'>
            ...
            <DrawerBody>
              <form onSubmit={tierUpdate} style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                <FormControl w='100%'>
                  <FormLabel>
                    Tier Type Update <Box as='span' color='orange'>*</Box>
                  </FormLabel>
      ...
      <Box display='flex' justifyContent='space-between' alignItems='center'>
        ...
        <Box>
          ...
          <Button
            ...
            _active={{ bgGradient: 'linear(180deg, #2267A2 0%, #0D4675 100%)' }}
            _focus={{ bgGradient: 'linear(180deg, #2267A2 0%, #0D4675 100%)' }}
            onClick={() => navigate("/master-setting/add-new-type")}
          >
      ...
      <Box className="card p-fluid">
        {loader ?
          ...
          <DataTable value={products} header={header} filters={filters} onFilter={(e) => setFilters(e.filters)} dataKey="id" tableStyle={{ minWidth: '50rem' }}>
            <Column field="type" header="Category/Type" sortable bodyStyle={{ textAlign: 'center' }} style={{ width: '25%' }}></Column>
