...
const FoodLimits = () => {
  ...
  function toastCall() {
    return (toast({
      title: 'Food Limit Updated Sucessfully',
      status: 'success',
      duration: 3000,
      isClosable: true,
    }))
  ...
  useEffect(() => {
    const travelLimit = async () => {
      try {
        ...
        const response2 = await fetch(`${process.env.REACT_APP_API_URL}/travel-food`, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
          },
        ...
        if (response1.ok && response2.ok) {
          ...
          setTravelLimit(tableInputData.data.food_limit);
          setEmpGread(tableColumn.data.grade_values);
          buildMyInputObj(tableInputData.data.food_limit);
          setLoader(false)
        } else {
          navigate('/login')
        }
  ...
  const travelLimitPost = async (e) => {
    ...
    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/travel-food-post`,
      {
        method: "POST",
        body: formData,
        headers: {
  ...
  return (
    <Box>
      ...
      <Box bg='white' borderRadius='15px' mb='30px'>
        <Heading mb='15px' color='claimzTextBlueColor'>List of all Food Limits</Heading>
        {loader ?
