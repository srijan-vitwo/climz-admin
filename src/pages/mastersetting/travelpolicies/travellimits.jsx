...
const TravelLimits = () => {
  ...
  useEffect(() => {
    const travelLimit = async () => {
      try {
        setLoader(true)
        const response1 = await fetch(`${process.env.REACT_APP_API_URL}/mode-of-travel`, {
          method: "GET",
          headers: {
        ...
        const response2 = await fetch(`${process.env.REACT_APP_API_URL}/travel-travel`, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const buildMyInputObj = (tableInputData1) => {
    ...
    tableInputData.map((row, index) => {
      tempObj[`c-${row.grade_id}-${row.travel_mode_id}`] = row.limit;
    });
  ...
  const travelLimitPost = async (e) => {
    ...
    Object.entries(empTravelLimitInput).map(([indexKey, value]) => {
      formObj.push({
        ...
        travel_mode_id: indexKey.split("-")[2],
        limit: value
      });
    });
    console.log(formObj);
    ...
    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/travel-travel-post`,
      {
        method: "POST",
        body: formData,
        headers: {
  ...
  return (
    <Box>
      ...
      <Box bg='white' borderRadius='15px' mb='30px'>
        <Heading mb='15px' color='claimzTextBlueColor'>List of all Travel Limits</Heading>
        {loader ?
                  ...
                  >
                    <Tr>
                      ...
                      {travelMaster?.map((data, index) => {
                        return <Th key={index} color='white' p='15px 0px' textAlign='center' fontSize='1.4rem'>{data.name}</Th>
                      })}
                    </Tr>
                  </Thead>
                  <Tbody>
