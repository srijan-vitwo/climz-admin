...
const BusinessLocationDatatable = () => {
    ...
    useEffect(() => {
        const formDataValue = async () => {
            try {
                setLoader(true)
                const response1 = await fetch(`${process.env.REACT_APP_API_URL}/get-office-details`, {
                    method: "GET",
                    headers: {
    ...
    const RenderHeader = () => {
        ...
        const addCompanyLocation = async (e) => {
            ...
            try {
                setIsLoading(true)
                const response = await fetch(`${process.env.REACT_APP_API_URL}/add-office-details`, {
                    method: "POST",
                    body: formData,
                    headers: {
                        Authorization: `Bearer ${token}`,
        ...
        return (
            <Box display='flex' justifyContent='space-between' alignItems='center'>
                <Box w='100%' display='flex' justifyContent='space-between' alignItems='center'>
                    <Box borderBottom='3px solid var(--chakra-colors-claimzBorderColor)' width='310px !important' pb='10px' >
                    ...
                    <Box as='span' w='40%' className="p-input-icon-left" display='flex' alignItems='center'>
                        <i style={{ lineHeight: 1.5 }} className="pi pi-search" />
                        <Input pl='24px' type="search" value={globalFilterValue} onChange={onGlobalFilterChange} placeholder="Global Search" w='100%' />
                    </Box>
                </Box>
                <Box display='flex' justifyContent='space-between' alignItems='center'>
                    <Button
                            ...
                            <DrawerHeader pt='28px'>
                                <Box
                                    ...
                                    <Text background='linear-gradient(180deg, #2770AE 0%, #01325B 100%)'
                                        ...
                                        fontWeight='700'
                                        fontSize='28px'
                                        lineHeight='36px'>Add Business Location</Text>
                                </Box >
                            </DrawerHeader>
                            <DrawerBody>
                                <form onSubmit={addCompanyLocation} style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                                    <FormControl w='100%' mb='15px'>
                                        <FormLabel>
                                            Enter Latitude<Box as='span' color='orange'>*</Box>
                                        </FormLabel>
                                        <Input type='text' value={latitude} onChange={(e) => setLatitude(e.target.value)} required />
                                    </FormControl>
                                    <FormControl w='100%' mb='15px'>
                                        <FormLabel>
                                            Enter Longitude<Box as='span' color='orange'>*</Box>
                                        </FormLabel>
                                        <Input type='text' value={longitude} onChange={(e) => setLongitude(e.target.value)} required />
                                    </FormControl>
                                    <FormControl w='100%' mb='15px'>
                                        <FormLabel>
                                            Enter Radius<Box as='span' color='orange'>*</Box>
                                        </FormLabel>
                                        <Input type='text' value={radius} onChange={(e) => setRadius(e.target.value)} required />
                                    </FormControl>
                                    <FormControl w='100%'>
                                        <FormLabel >
                                            Select Location
    ...
    const DetailsTemplate = (rowData) => {
        ...
        const [id, setId] = useState(rowData.company_address_id)
        const { isOpen, onOpen, onClose } = useDisclosure()
                        ...
                        <DrawerHeader pt='28px'>
                            <Box
                                ...
                                <Text background='linear-gradient(180deg, #2770AE 0%, #01325B 100%)'
                                    ...
                                    fontWeight='700'
                                    fontSize='28px'
                                    lineHeight='36px'>Business Locations List Update</Text>
                            </Box >
                        </DrawerHeader>

                        <DrawerBody>
                            <form onSubmit={updateCompanyLocation} style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                                <FormControl w='100%' mb='15px'>
                                    <FormLabel>
                                        Enter Latitude<Box as='span' color='orange'>*</Box>
                                    </FormLabel>
    ...
    return (
        ...
        >
            {loader ?
                ...
                :
                <Box className="card">
                    <DataTable value={empList?.address} header={Header} filters={filters} onFilter={(e) => setFilters(e.filters)} dataKey="company_address_id" >
                        <Column header="Location" body={LocationTemplate} bodyStyle={{ textAlign: 'center' }} />
                        <Column header="Company Lat" field='company_lat' bodyStyle={{ textAlign: 'center' }} />
                        <Column header="Company Lng" field='company_lng' bodyStyle={{ textAlign: 'center' }} />
                        <Column header="Company Radius" field='radius' bodyStyle={{ textAlign: 'center' }} />
                        <Column header="Edit" body={DetailsTemplate} bodyStyle={{ textAlign: 'center' }} />
                    </DataTable>
                </Box>
            }
        </CssWrapper>
