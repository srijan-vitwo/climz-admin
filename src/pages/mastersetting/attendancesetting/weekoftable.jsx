...
const WeekOfTable = ({ Msg }) => {
    ...
    useEffect(() => {
        const weekOffTable = async () => {
            try {
                setLoader(true)
                const response1 = await fetch(`${process.env.REACT_APP_API_URL}/weekoff`, {
                    method: "GET",
                    headers: {
    ...
    const ActionTemplate = (rowData) => {
        ...
        const updateVariant = async (e) => {
            ...
            try {
                setIsLoading(true)
                const response = await fetch(`${process.env.REACT_APP_API_URL}/weekoff-update`, {
                    method: "POST",
                    body: formData,
                    headers: {
                        Authorization: `Bearer ${token}`,
                        ...
                        <DrawerHeader pt='28px'>
                            <Box
                                ...
                                <Text background='linear-gradient(180deg, #2770AE 0%, #01325B 100%)'
                                    ...
                                    fontWeight='700'
                                    fontSize='28px'
                                    lineHeight='36px'>Update Variant Details</Text>
                            </Box >
                        </DrawerHeader>

                        <DrawerBody>
                            <Box display='flex' flexDirection='column' alignItems='end'>
                                <form style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'end' }} onSubmit={updateVariant}>
                                    <FormControl mb='10px'>
                                        <FormLabel>Enter Variant Name</FormLabel>
                                        <Input type='text' value={variantName} placeholder="Enter Variant Name" onChange={e => setVariantName(e.target.value)} required />
                                    </FormControl>
                                    <Box w='100%' mb='10px' display='flex' justifyContent='space-between' alignItems='center'>
                                        <FormControl width='48%'
                                            ...
                                            <MultiSelect value={selectedDay} onChange={(e) => setSelectedDay(e.value)} options={day}
                                                placeholder="Select Regular Variant" maxSelectedLabels={3} className="w-full md:w-20rem" />
                                        </FormControl>
                                        <FormControl width='48%'>
                                            <FormLabel>Alternative Variant</FormLabel>
                                            <Select color='#6c757d' placeholder='Select option' onChange={(event) =>
                                    ...
                                    <FormControl mb='10px' width='100%'
                                        ...
                                        <MultiSelect value={selectedWeeks} onChange={(e) => setSelectedWeeks(e.value)} options={weeks}
                                            placeholder="Select Alternate Week Off" maxSelectedLabels={3} className="w-full md:w-20rem" />
                                    </FormControl>
                                    <Button
                                        disabled={isLoading}
                                        isLoading={isLoading}
        ...
        <CssWrapper>
            ...
            >
                ...
                <div className="card p-fluid">
                    <DataTable value={variant} dataKey="variant_id" tableStyle={{ minWidth: '50rem' }}>
                        <Column field="variant_name" header="Variant Name" bodyStyle={{ textAlign: 'center' }} style={{ width: '25%' }}></Column>
                        <Column rowEditor header="Edit" body={ActionTemplate} headerStyle={{ width: '25%', minWidth: '8rem' }} bodyStyle={{ textAlign: 'center' }}></Column>
                    </DataTable>
                </div>
            }
        </CssWrapper >
