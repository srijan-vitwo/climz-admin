...
const SalaryDetails = () => {
    ...
    const separateTheComponents = (data) => {
        ...
        data.earning_components.map((row) => {
            ...
            tempEarning[row.salary_component_id]["inputPercentage"] = row.percentage;
        });
    ...
    const separateThedeductionComponents = (data) => {
        ...
        data.deduction_components.map((row) => {
            ...
            tempDeduction[row.salary_component_id]["inputPercentage"] = row.percentage;
        });
    ...
    const handleEarningInputChange = (key, e) => {
        ...
        const earningComponentsObj = JSON.parse(JSON.stringify(earningComponents));
        earningComponentsObj[key].input = inputVal;

        const parentKey = earningComponentsObj[key].percentage_of;
        const parentValue = parentKey > 0 ? earningComponentsObj[parentKey].input : CTC;
        earningComponentsObj[key].inputPercentage = ((inputVal / parentValue) * 100).toFixed(2);
        ...
        const updateMyChildrensInput = (key, childrenList) => {
            ...
            childrenList.map((childrenKey) => {
                ...
                earningComponentsObj[childrenKey].input = ((parentValue * earningComponentsObj[childrenKey].percentage) / 100).toFixed(2);
                earningComponentsObj[childrenKey].inputPercentage = earningComponentsObj[childrenKey].percentage;
                updateMyChildrensInput(childrenKey, findMyChildrenList(childrenKey));
            });
        }
    ...
    const calculateTheStateEarningValues = (ctc) => {
        ...
        Object.entries(earningComponentsObj).map(([key, data]) => {
            ...
            earningComponentsObj[key].input = ((principalAmount * data.percentage) / 100).toFixed(2);
    ...
    const handleInputChangeDeduction = (key, e) => {
        const inputVal = e.target.value;
        const deductionComponentsObj = JSON.parse(JSON.stringify(deductionComponents));
    ...
    const handleInputChangePerquisits = (key, e) => {
        const inputVal = e.target.value;
        const perquisitsComponentsObj = JSON.parse(JSON.stringify(perquisitsComponents));
    ...
    const handleInputChangeProfit = (key, e) => {
        const inputVal = e.target.value;
        const profitComponentsObj = JSON.parse(JSON.stringify(profitComponents));
    ...
    const EarningDetails = Object.entries(earningComponents).map(([key, data]) => {
        let dependOnTxt = "CTC";
        if (data.percentage_of > 0) {
        ...
        return (<FormControl>
            {data.fixed_component_marking === 1 || data.fixed_component_marking === 2 ? <FormLabel>{data.salary_component}{" "}
                {data.fixed_component_marking === 1 ? <Tooltip hasArrow label='This component is mark as besic' placement='right-end'>
                    <i className="fa-solid fa-circle-info"></i>
                </Tooltip> : <Tooltip hasArrow label='This component is mark as HRA' placement='right-end'>
                    <i className="fa-solid fa-circle-info"></i>
                </Tooltip>}
            </FormLabel> : <FormLabel>{data.salary_component} <Button onClick={() => handleDeleteEarning(key)} bg='none' _hover={{ bg: 'none' }} _active={{ bg: 'none' }}><i className="fa-solid fa-trash"></i></Button></FormLabel>}
            <Input
                type='number'
    ...
    const handleCheckboxClickPtax = async () => {
        ...
        try {
            const response1 = await fetch(`${process.env.REACT_APP_API_URL}/rule-two/${CTC}/${user.id}`, {
                method: "GET",
                headers: {
                    Authorization: `Bearer ${token}`,
                },
    ...
    const handleCheckboxClickPF = async () => {
        setIsChecked(!isChecked);
        try {
            const response1 = await fetch(`${process.env.REACT_APP_API_URL}/rule-three/${basic}`, {
                method: "GET",
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            ...
            if (response1.ok) {
                ...
                let tempData = {
                    ...
                    "Employer's contribution for Provident Fund": data["employer_contri"],
                    "Employee's contribution for Provident Fund": data["empolyee_contri"],
                }
    ...
    const handleCheckboxClickESIC = async () => {
        setIsChecked(!isChecked);
        try {
            const response1 = await fetch(`${process.env.REACT_APP_API_URL}/rule-one/${CTC}`, {
                method: "GET",
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            ...
            if (response1.ok) {
                ...
                setEsicValue(data)
                let tempData = {
                    "Employer's contribution for Employees' State Insurance Corporation": data["employer_contri"],
                    "Employee's contribution for Employees' State Insurance Corporation": data["empolyee_contri"],
                }
    ...
    const salaryDetailsSubmit = async (e) => {
        ...
        try {
            setIsLoading(true)
            const response = await fetch(`${process.env.REACT_APP_API_URL}/salary-details-submit`, {
                method: "POST",
                body: formData,
                headers: {
                    Authorization: `Bearer ${token}`,
    ...
    const filteredEmployeeOther = Object.fromEntries(
        Object.entries(deductionComponents).filter(([key, value]) => value.rule_id === 0)
    );
    const filteredEmployeeEsi = Object.fromEntries(
        Object.entries(deductionComponents).filter(([key, value]) => value.rule_id === 1)
    );
    const filteredEmployeePtax = Object.fromEntries(
        Object.entries(deductionComponents).filter(([key, value]) => value.rule_id === 2)
    );
    const filteredEmployeePf = Object.fromEntries(
        Object.entries(deductionComponents).filter(([key, value]) => value.rule_id === 3)
    );
    const filterMarkcomponent = Object.fromEntries(
        Object.entries(earningComponents).filter(([key, value]) => value.input > 15000)
    );
    const filterBasicComponent = Object.fromEntries(
        Object.entries(earningComponents).filter(([key, value]) => value.fixed_component_marking === 1)
    );
            ...
            <CardBody width='100%'>
                <Box>
                    <Box>
                        ...
                        <Grid templateColumns='repeat(3, 1fr)' gap={6}>
                            <FormControl>
                                ...
                                <Input type='text' placeholder='Employee Name' value={employeeName} onChange={(e) => setEmployeeName(e.target.value)} readOnly />
                            </FormControl>
                            <FormControl>
                                <FormLabel>Employee Code</FormLabel>
                                <Input type='text' placeholder='Employee Code' value={employeeCode} onChange={(e) => setEmployeeCode(e.target.value)} readOnly />
                            </FormControl>
                            <FormControl>
                                <FormLabel>Date of Joining</FormLabel>
                                <Input type='date' placeholder='Date of Joining' value={dateOfJoining} onChange={(e) => setDateOfJoining(e.target.value)} readOnly />
                            </FormControl>
                            <FormControl>
                                <FormLabel>Account Number</FormLabel>
                                <Input type='text' placeholder='Account Number' value={accountNumber} onChange={(e) => setAccountNumber(e.target.value)} readOnly />
                            </FormControl>
                            <FormControl>
                                <FormLabel>PF A/C Number</FormLabel>
                                <Input type='text' placeholder='PF A/C Number' value={PFNumber} onChange={(e) => setPFNumber(e.target.value)} />
                            </FormControl>
                            <FormControl>
                                <FormLabel>UAN Number</FormLabel>
                                <Input type='text' placeholder='UAN Number' value={UANNumber} onChange={(e) => setUANNumber(e.target.value)} />
                            </FormControl>
                            <FormControl>
                                <FormLabel>ESI Number</FormLabel>
                                <Input type='text' placeholder='ESI Number' value={ESINumber} onChange={(e) => setESINumber(e.target.value)} />
                            </FormControl>
                            <FormControl>
                            ...
                            <FormControl>
                                ...
                                <Input type='file' placeholder="Logo"
                                    p='0px' onChange={(e) => setCompanyLogo(e.target.files[0])} sx={{
                            ...
                            <Divider orientation='vertical' />
                            <FormControl>
                                <FormLabel fontSize='1.8rem !important'
                                    fontWeight='600'
                                    color='var(--chakra-colors-claimzTextBlueColor)'
                                > Monthly CTC :</FormLabel>
                                <Input type='number' placeholder='Enter Monthly CTC' className='handleScroll' value={CTC} onChange={handleCTC} />
                            </FormControl>
                            {/* <p>Total: <span>{CTC - totalEarningSum.toFixed(2)}</span></p> */}
                            {Object.keys(filterMarkcomponent).length >= 1 &&
                                <Box p='5px 15px' borderRadius='15px'
                                    boxShadow='rgba(0, 0, 0, 0.24) 0px 3px 8px'>
                                    <FormControl>
                                        <FormLabel>Provident Fund Capping :</FormLabel>
                                            <RadioGroup onChange={setCapping} value={capping}>
                                                <Stack direction='row' gap='20px'>
                                                    <Radio value='1' size='lg'>
                                                        <Text fontWeight='600' color='claimzTextBlueLightColor'>15000</Text>
                                                    </Radio>
                                                    <Radio value='2' size='lg'>
                                                        <Text fontWeight='600' color='claimzTextBlueLightColor'>{filterBasicComponent[2]["input"]}</Text>
                                                    </Radio>
                                                    <Radio value='0' size='lg'>
                                                        <Text fontWeight='600' color='claimzTextBlueLightColor'>0</Text>
                                                    </Radio>
                                                </Stack>
                                            </RadioGroup>
                    ...
                    <Box>
                        ...
                        <Box display='flex' gap='1%'>
                            {Object.keys(filteredEmployeeEsi).length > 0 ?
                                <Box boxShadow='rgba(0, 0, 0, 0.24) 0px 3px 8px' w='32%' p='15px' borderRadius='5px'>
                                    <Box display='flex' gap='10px' alignItems='center' mb='15px' color='#083d6acc !important'>
                                        <i className="fa-solid fa-circle-info"></i>
                                        <Checkbox
                                            size='lg'
                                            type="checkbox"
                                        ...
                                        >
                                            <Text
                                                fontWeight='600'
                                            >ESIC
                                                <Button onClick={() => handleDeleteObjects(1)} bg='none' _hover={{ bg: 'none' }} _active={{ bg: 'none' }}><i className="fa-solid fa-trash"></i></Button>
                                            </Text>
                                        </Checkbox>
                                        <Box fontSize='1.4rem' color='red' fontWeight='600' >
                                            {esicValue?.data}
                                        </Box>
                                    </Box>

                                    {
                                        Object.entries(filteredEmployeeEsi).map(([key, value]) => (
                                            <Box key={key}>
                                                <FormControl mb='15px' >
                                                    <FormLabel>{value?.salary_component} </FormLabel>
                                                    <Input type='number' placeholder={value?.salary_component} name={value?.salary_component} value={empEsiValue[value?.salary_component]} onChange={handleInputChangeDeduction} readOnly />
                                                </FormControl>
                                            </Box>
                                        ))
                                    }
                            ...
                            {Object.keys(filteredEmployeePtax).length > 0 ?
                                <Box boxShadow='rgba(0, 0, 0, 0.24) 0px 3px 8px' w='32%' p='15px' borderRadius='5px'>
                                    <Box display='flex' gap='10px' alignItems='center' mb='15px' color='#083d6acc !important'>
                                        ...
                                        >
                                            <Text fontWeight='600'>PTAX <Button onClick={() => handleDeleteObjects(2)} bg='none' _hover={{ bg: 'none' }} _active={{ bg: 'none' }}><i className="fa-solid fa-trash"></i></Button></Text>
                                        </Checkbox>
                                    </Box>
                                    {
                                        Object.entries(filteredEmployeePtax).map(([key, value]) => (
                                            <Box key={key}>
                                                <FormControl mb='15px' >
                                                    <FormLabel>{value?.salary_component} </FormLabel>
                                                    <Input type='number' placeholder={value?.salary_component} name={value?.salary_component} value={pTaxValue} onChange={handleInputChangeDeduction} readOnly />
                                                </FormControl>
                                            </Box>
                                        ))
                                    }
                            ...
                            {Object.keys(filteredEmployeePf).length > 0 ?
                                <Box boxShadow='rgba(0, 0, 0, 0.24) 0px 3px 8px' w='32%' p='15px' borderRadius='5px'>
                                    <Box display='flex' gap='10px' alignItems='center' mb='15px' color='#083d6acc !important'>
                                        ...
                                        >
                                            <Text fontWeight='600'>PF <Button onClick={() => handleDeleteObjects(3)} bg='none' _hover={{ bg: 'none' }} _active={{ bg: 'none' }}><i className="fa-solid fa-trash"></i></Button></Text>
                                        </Checkbox>
                                    </Box>

                                    {
                                        Object.entries(filteredEmployeePf).map(([key, value]) => (
                                            <Box key={key}>
                                                <FormControl mb='15px' >
                                                    <FormLabel>{value?.salary_component} </FormLabel>
                                                    <Input type='number' placeholder={value?.salary_component} name={value?.salary_component} value={empPfValue[value?.salary_component]} onChange={handleInputChangeDeduction} readOnly />
                                                </FormControl>
                                            </Box>
                                        ))
                                    }
                        ...
                        <Grid templateColumns='repeat(3, 1fr)' gap={6} boxShadow='rgba(0, 0, 0, 0.24) 0px 3px 8px' p='15px' mt='24px' borderRadius='5px'>
                            {
                                Object.entries(filteredEmployeeOther).map(([key, value]) => (
                                    <Box key={key}>
                                        <FormControl>
                                            <FormLabel>{value?.salary_component} <Button onClick={() => handleDeleteDeduction(key)} bg='none' _hover={{ bg: 'none' }} _active={{ bg: 'none' }}><i className="fa-solid fa-trash"></i></Button></FormLabel>
                                            <Input type='number' placeholder={value?.salary_component} name={value?.salary_component} value={deductionformData.data?.salary_component} onChange={(e) => handleInputChangeDeduction(value?.salary_component_id, e)} />
                                        </FormControl>
                                    </Box>
                                ))
                            }
                    ...
                    <Box>
                        ...
                        <Grid templateColumns='repeat(3, 1fr)' gap={6}>
                            {
                                empDetails?.perquisits_components?.map((data, index) => (
                                    <Box key={index}>
                                        <FormControl>
                                            <FormLabel>{data?.salary_component}</FormLabel>
                                            <Input type='number' placeholder={data?.salary_component} name={data?.salary_component} value={deductionformData.data?.salary_component} onChange={e => handleInputChangePerquisits(data.salary_component_id, e)} />
                                        </FormControl>
                                    </Box>
                                ))
                            }
                        ...
                        <Grid templateColumns='repeat(3, 1fr)' gap={6}>
                            {
                                empDetails?.profit_components?.map((data,index) => (
                                    <FormControl key={index}>
                                        <FormLabel>{data?.salary_component}</FormLabel>
                                        <Input type='number' placeholder={data?.salary_component} name={data?.salary_component} value={deductionformData.data?.salary_component} onChange={(e) => handleInputChangeProfit(data.salary_component_id, e)} />
                                    </FormControl>
                                ))
                            }
                        </Grid>
