...
const TravelMaster = () => {
    ...
    useEffect(() => {
        const departmentList = async () => {
            try {
                setLoader(true)
                const response1 = await fetch(`${process.env.REACT_APP_API_URL}/convayence-mode-of-travel`, {
                    method: "GET",
                    headers: {
    ...
    const RenderHeader = () => {
        ...
        const travelMasterAdd = async (e) => {
            ...
            try {
                setIsLoading(true)
                const response = await fetch(`${process.env.REACT_APP_API_URL}/convayence-mode-of-travel-post`, {
                    method: "POST",
                    body: formData,
                    headers: {
                        Authorization: `Bearer ${token}`,
                ...
                <Box>
                    ...
                    <Modal onClose={onClose} isOpen={isOpen} isCentered>
                        ...
                        <ModalContent minW='600px' minH='400px'>
                            ...
                            <ModalCloseButton color='white' size='lg' mt='10px' _hover={{ bg: 'none' }} _active={{ bg: 'none' }} />
                            <ModalBody px='24px'>
    ...
    const ActionTemplate = (rowData) => {
        const toast = useToast()
        const [updateComponentName, setUpdateComponentName] = useState(rowData.component_name)
        ...
        const tierUpdate = async (e) => {
            ...
            try {
                setIsLoading(true)
                const response = await fetch(`${process.env.REACT_APP_API_URL}/convayence-mode-of-travel-update`, {
                    method: "POST",
                    body: formData,
                    headers: {
                        Authorization: `Bearer ${token}`,
                        ...
                        <DrawerHeader pt='28px'>
                            <Box
                                ...
                                <Text background='linear-gradient(180deg, #2770AE 0%, #01325B 100%)'
                                    ...
                                    fontWeight='700'
                                    fontSize='28px'
                                    lineHeight='36px'> Tier Master Item Update</Text>
                            </Box >
                        </DrawerHeader>
                        ...
                        <DrawerBody>
                            <form onSubmit={tierUpdate} style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                                <FormControl w='100%' mb='15px'>
                                    ...
                                    <Input type='text' value={updateComponentName} onChange={(e) => setUpdateComponentName(e.target.value)} required />
                                </FormControl>
                                <FormControl w='100%'>
                                    <FormLabel>
                                        Enter Mode Of Type <Box as='span' color='orange'>*</Box>
                                    </FormLabel>
                                    <Input type='text' value={updateType} onChange={(e) => setUpdateType(e.target.value)} required />
                                </FormControl>
                                <Button
                                    disabled={isLoading}
                                    isLoading={isLoading}
        ...
        <CssWrapper>
            ...
            >
                ...
                <div className="card p-fluid">
                    <DataTable value={products} header={header} filters={filters} onFilter={(e) => setFilters(e.filters)} editMode="row" dataKey="conveyance_mode_id" tableStyle={{ minWidth: '50rem' }}>
                        <Column field="component_name" header="Modes" sortable bodyStyle={{ textAlign: 'center' }} style={{ width: '33%' }}></Column>
                        <Column field="type" header="Type" sortable bodyStyle={{ textAlign: 'center' }} style={{ width: '33%' }}></Column>
                        <Column header="Action" body={ActionTemplate} headerStyle={{ width: '33%', minWidth: '8rem' }} bodyStyle={{ textAlign: 'center' }}></Column>
                    </DataTable>
                </div>
            }
        </CssWrapper>
