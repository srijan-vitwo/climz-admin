...
import { Box, Image, Button, Input, Heading, Table, Tbody, Td, Th, Thead, Tr, useToast } from "@chakra-ui/react";
...
const TravelLimit = () => {
  ...
  useEffect(() => {
    const travelLimit = async () => {
      try {
        ...
        const response2 = await fetch(`${process.env.REACT_APP_API_URL}/convayence-travel`, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const buildMyInputObj = (tableInputData1) => {
    ...
    tableInputData.map((row, index) => {
      tempObj[`c-${row.grade_id}-${row.conveyance_mode_id}`] = row.limit_per_km;
    });
  ...
  const travelLimitPost = async (e) => {
    ...
    try {
      setIsLoading(true)
      const response = await fetch(`${process.env.REACT_APP_API_URL}/convayence-travel-post`, {
        method: "POST",
        body: formData,
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  return (
    <Box>
      <Box bg='white' borderRadius='15px' mb='30px'>
        ...
        {loader ?
          <Box height='calc(100vh - 274px)' display='flex'
            alignItems='center'
