import { Box, Text, Button, Table, Thead, Tbody, Tr, Th, Td, Input, useToast } from '@chakra-ui/react'
...
const IncidentalLimit = () => {
    ...
    useEffect(() => {
        const convayenceTravelLimit = async () => {
            try {
                const response1 = await fetch(`${process.env.REACT_APP_API_URL}/convayence-incidental`, {
                    method: "GET",
                    headers: {
    ...
    const incidentalLimitAdd = async (e) => {
        ...
        try {
            setIsLoading(true)
            const response = await fetch(`${process.env.REACT_APP_API_URL}/convayence-incidental-post`, {
                method: "POST",
                body: formData,
                headers: {
                    Authorization: `Bearer ${token}`,
