...
const OnboardingCandidate = () => {
    ...
    useEffect(() => {
        const formDataValue = async () => {
            try {
                setLoader(true)
                const response1 = await fetch(`${process.env.REACT_APP_API_URL}/get-candidates/${rows}?page=${first}`, {
                    method: "GET",
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
    ...
    const RenderHeader = () => {
        ...
        return (
            <Box display='flex' justifyContent='space-between' alignItems='center'>
                <Box width='40%' className="flex flex-wrap gap-2 justify-content-between align-items-center">
                            ...
                            <DrawerHeader pt='28px'>
                                <Box
                                    ...
                                    <Text background='linear-gradient(180deg, #2770AE 0%, #01325B 100%)'
                                        ...
                                        fontWeight='700'
                                        fontSize='28px'
                                        lineHeight='36px'>Onboarding<br /> Process Request</Text>
                                </Box >
                            </DrawerHeader>
                            <DrawerBody>
                                <form style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }} onSubmit={addCandidate}>
                                    <FormControl w='100%' mb='15px'>
                                        <FormLabel>
                                            Enter Full Name <Box as='span' color='orange'>*</Box>
                                        </FormLabel>
                                        <Input type='text' onChange={(e) => setName(e.target.value)} required />
                                    </FormControl>
                                    <FormControl w='100%' mb='15px'>
                                        <FormLabel>
                                            Enter Mobile Number <Box as='span' color='orange'>*</Box>
                                        </FormLabel>
                                        <Input type='number' onChange={(e) => setNumber(e.target.value)} required />
                                    </FormControl>
                                    <FormControl w='100%' mb='15px'>
                                        <FormLabel>
                                            Register Mail ID<Box as='span' color='orange'>*</Box>
                                        </FormLabel>
                                        <Input type='email' onChange={(e) => setMail(e.target.value)} required />
                                    </FormControl>
                                    <FormControl w='100%'>
                                        <FormLabel>
                                            Expected Joining Date <Box as='span' color='orange'>*</Box>
                                        </FormLabel>
    ...
    const DetailsTemplate = (rowData) => {
        return (
            <OnbordingDrawer rowData={rowData} fromValue={fromValue} empUser={empUser} />
        )
    };
        ...
        return (
            <Box>
                {rowData.candidate_status === 1 ?
                    <Tooltip hasArrow fontSize='12px' p='5px' label='Candidate Seen The Invition' >
                        <Box
                    ...
                    :
                    <Tooltip fontSize='12px' p='5px' hasArrow label="Candidate did't Seen The Invition">
                        <Box
                    ...
                    >
                        <Box className="card">
                            <DataTable value={empList?.data} header={Header} filters={filters} onFilter={(e) => setFilters(e.filters)} dataKey="id" >
                                ...
                                <Column header="Doc Status" body={DocStatus} bodyStyle={{ textAlign: 'center' }} />
                                <Column header="Details" body={DetailsTemplate} bodyStyle={{ textAlign: 'center' }} />
                                <Column header="Signup Status" body={StatusTemplate} bodyStyle={{ textAlign: 'center' }} />
                            </DataTable>
                            <Box display='flex' justifyContent='flex-end' backgroundColor='white'>
                                <Paginator first={first} rows={rows} totalRecords={empList?.total} rowsPerPageOptions={[5, `${empList?.total}`]} onPageChange={onPageChange} />
                            </Box>
                        </Box>
                    </Box>
                }
