...
const AssetsList = () => {
    ...
    useEffect(() => {
        const departmentList = async () => {
            try {
                setLoader(true)
                const response1 = await fetch(`${process.env.REACT_APP_API_URL}/asset-list/${rows}?page=${first}`, {
                    method: "GET",
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                });
                const response2 = await fetch(`${process.env.REACT_APP_API_URL}/emp-list`, {
                    method: "GET",
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                });
                const response3 = await fetch(`${process.env.REACT_APP_API_URL}/uom-master`, {
                    method: "GET",
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
        ...
        return (
            <Box display='flex' justifyContent='space-between' alignItems='center'>
                <Box as='span' className="p-input-icon-left" display='flex' alignItems='center'>
                    ...
                    <Input pl='24px' type="search" value={value || ''} onChange={(e) => onGlobalFilterChange(e)} placeholder="Global Search" w='50%' />
                </Box>
                <Box>
    ...
    const UpdateTemplate = (rowData) => {
        ...
        const updateAssect= async (e) => {
            ...
            try {
                setIsLoading(true)
                const response = await fetch(`${process.env.REACT_APP_API_URL}/asset-update`, {
                    method: "POST",
                    body: formData,
                    headers: {
                        Authorization: `Bearer ${token}`,
                ...
                >
                    ...
                    <DrawerContent maxW='40% !important' bgGradient='linear(180deg, #DCF9FF 0%, #FFFFFF 100%)'>
                        ...
                        <DrawerHeader pt='28px'>
                            <Box
                                borderBottom='3px solid var(--chakra-colors-claimzBorderColor)' width='350px' pb='10px'>
                                <Text background='linear-gradient(180deg, #2770AE 0%, #01325B 100%)'
                                    backgroundClip='text'
                                    fontWeight='700'
                                    fontSize='28px'
                                    lineHeight='36px'>Asset Details Update</Text>
                            </Box >
                        </DrawerHeader>
                        <DrawerBody>
                                ...
                                <Box w='100%'>
                                    <Box width='100%' paddingBottom='20px'>
                                            ...
                                            <FormControl mb='10px'>
                                                ...
                                                <Textarea value={description} border='1px solid var(--chakra-colors-claimzBorderGrayColor)' _hover={{border:'1px solid var(--chakra-colors-claimzBorderGrayColor)'}} type='text' onChange={(e) => setDescription(e.target.value)} h='100px' fontSize='1.4rem' />
                                            </FormControl>
                                            <FormLabel>Base UOM?</FormLabel>
                                            <Select placeholder='Select option' value={uomId} onChange={(e) => setUomId(e.target.value)}>
                                                {uom?.map((data, index) => {
    ...
    const ActionTemplate = (rowData) => {
        ...
        const assectAssign = async (e) => {
            ...
            try {
                setIsLoading(true)
                const response = await fetch(`${process.env.REACT_APP_API_URL}/asset-assign-post`, {
                    method: "POST",
                    body: formData,
                    headers: {
                        Authorization: `Bearer ${token}`,
                        ...
                        <DrawerBody>
                            ...
                            {rowData?.specifications[0]?.specification.length > 0 ? 
                                ...
                                <Box w='100%' textAlign='center'>
                                    <Text as="span" fontSize='1.4rem' fontWeight='500' color='claimzTextBlackColor' textAlign='center'>Specification Not Added !</Text>
                                </Box>
                            }
                        </DrawerBody>
                    </DrawerContent>
        ...
        <CssWrapper>
            ...
            >
                ...
                <Box className="card p-fluid">
                    <DataTable value={products?.data} header={header} filters={filters} onFilter={(e) => setFilters(e.filters)} dataKey="asset_id" tableStyle={{ minWidth: '50rem' }}>
                        <Column field="asset_name" header="Asset Name " sortable bodyStyle={{ textAlign: 'center' }} style={{ width: '20%' }}></Column>
                        <Column field="group_description" header="Group Description" sortable bodyStyle={{ textAlign: 'center' }} style={{ width: '40%' }}></Column>
                        <Column field="group_name" header="Group Name" bodyStyle={{ textAlign: 'center' }} style={{ width: '20%' }}></Column>
                        <Column header="Edit" body={UpdateTemplate} style={{ width: '10%' }} bodyStyle={{ textAlign: 'center' }}></Column>
                        <Column header="Action" body={ActionTemplate} style={{ width: '10%' }} bodyStyle={{ textAlign: 'center' }}></Column>
                    </DataTable>
                    <Box display='flex' justifyContent='flex-end' backgroundColor='white'>
                        <Paginator first={first} rows={rows} totalRecords={products?.total} rowsPerPageOptions={[`${products?.total}`]} onPageChange={onPageChange} />
                    </Box>
                </Box>
            }
        </CssWrapper >
