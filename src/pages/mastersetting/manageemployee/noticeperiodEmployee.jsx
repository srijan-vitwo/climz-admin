...
const NoticePeriodEmployee = () => {
    ...
    useEffect(() => {
        ...
        const formDataValue = async () => {
            try {
                setLoader(true)
                const response1 = await fetch(`${process.env.REACT_APP_API_URL}/approved-resigned-employee`, {
                    method: "GET",
                    headers: {
                        ...
                        <DrawerBody>
                            ...
                            >
                                {fromLoader ?
                                    ...
                                    <EmployeeUpdate details={rowData.id} fromValue={fromValue} userData={userData} FromLoader={fromLoader} />}
                            </Box>
                        </DrawerBody>
                    </DrawerContent>
                </Drawer>
    ...
    const ResignTemplate = (rowData) => {
        const token = localStorage.getItem("token");
        const [userId, setuserId] = useState(rowData.id)
        const toast = useToast()

        function toastCallApproveResignationSucess() {
            return (toast({
                title: 'Resignation Approve Sucessfully',
                status: 'success',
                duration: 3000,
                isClosable: true,
            }))
        }
        function toastCallApproveResignationFaild() {
            return (toast({
                title: 'Resignation Approve Faild',
                status: 'error',
                duration: 3000,
                isClosable: true,
            }))
        ...
        const ApproveResignation = async (e) => {
            e.preventDefault();
            const response = await fetch(
                `${process.env.REACT_APP_API_URL}/resign-approval/${userId}`,
                {
                    method: "GET",
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            const data = await response.json();
            if (response.status === 200) {
                toastCallApproveResignationSucess()
            } else {
        ...
        return (
            <Box>
                ...
                {rowData.resign_aproval == '1' && <Button>Pending</Button>}
                {rowData.resign_aproval == '2' && <Button onClick={ApproveResignation}>Approve</Button>}
            </Box>
        )
    }
    ...
    return (
        <CssWrapper >
            {loader ?
                <Box height='calc(100vh - 240px)' display='flex'
                    alignItems='center'
                    justifyContent='center'>
                    <Image src={Loader} alt='Loader' />
                </Box> :
                <Box className="card">
                    <DataTable value={empList} header={header} filters={filters} onFilter={(e) => setFilters(e.filters)} dataKey="id" >
                        {cols.map((col, index) => (
                        ...
                        <Column header="Resignation status" body={ResignTemplate} bodyStyle={{ textAlign: 'center' }} style={{ width: '14%' }} />
                    </DataTable>
                    {/* pegination */}
...
export default NoticePeriodEmployee