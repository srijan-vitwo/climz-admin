...
const ProbationEmployee = () => {
    ...
    useEffect(() => {
        ...
        const formDataValue = async () => {
            try {
                setLoader(true)
                const response1 = await fetch(`${process.env.REACT_APP_API_URL}/probation-employee`, {
                    method: "GET",
                    headers: {
    ...
    const ActionTemplate = (rowData) => {
        ...
        const probationToPermanent = async () => {
            try {
                const response = await fetch(`${process.env.REACT_APP_API_URL}/convert-to-permanat-employee/${userId}`, {
                    method: "GET",
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                ...
                >
                    ...
                    <DrawerContent maxW='40% !important' bgGradient='linear(180deg, #DCF9FF 0%, #FFFFFF 100%)'>
                        ...
                        <DrawerHeader pt='28px'>
                            <Box
                                borderBottom='3px solid var(--chakra-colors-claimzBorderColor)' width='300px' pb='10px' mb='15px'>
                                <Text background='linear-gradient(180deg, #2770AE 0%, #01325B 100%)'
                                    backgroundClip='text'
                                    fontWeight='700'
                                    fontSize='28px'
                                    lineHeight='36px'>{rowData.emp_name}  Details</Text>
                            </Box >
                        </DrawerHeader>
                        ...
                        <DrawerBody>
                            <Box display='flex' justifyContent='flex-end' mb='20px'>
                                <Button onClick={LetterOnOpen} mr='15px' fontSize='1.4rem' padding='20px 10px' background='var(--chakra-colors-claimzMainGeadientColor)' color='white' display='flex' alignItems='center' justifyContent='center' borderRadius='50px' _hover={{ background: 'var(--chakra-colors-claimzMainGeadientColor)' }} _active={'none'}>
                                    <i className="fa-solid fa-circle-exclamation"></i><Text ml='5px' fontSize='1.4rem' fontWeight='600'>Warning </Text>
                                </Button>
                                <Button onClick={LetterOnOpen} mr='15px' fontSize='1.4rem' padding='20px 10px' background='var(--chakra-colors-claimzMainGeadientColor)' color='white' display='flex' alignItems='center' justifyContent='center' borderRadius='50px' _hover={{ background: 'var(--chakra-colors-claimzMainGeadientColor)' }} _active={'none'}>
                                    <i className="fa-solid fa-envelope"></i><Text ml='5px' fontSize='1.4rem' fontWeight='600'>Terminate </Text>
                                </Button>
                                <Button onClick={probationToPermanent} padding='20px 10px' fontSize='1.4rem' background='var(--chakra-colors-claimzMainGeadientColor)' color='white' display='flex' alignItems='center' justifyContent='center' borderRadius='50px' _hover={{ background: 'var(--chakra-colors-claimzMainGeadientColor)' }} _active={'none'}>
                                    <i className="fa-solid fa-user-tie"></i> <Text ml='5px' fontSize='1.4rem' fontWeight='600'>Convert </Text>
                                </Button>
                            </Box>
                            ...
                            <Box p='10px' mb='15px'>
                                ...
                                <Text mb='10px' fontSize='1.6rem' fontWeight='600' color='claimzTextBlueColor'>Mobile No - <Box as="span" color='claimzTextBlackColor'>{rowData.mobile_no}</Box></Text>
                                <Text mb='10px' fontSize='1.6rem' fontWeight='600' color='claimzTextBlueColor'>Emg Mobile No - <Box as="span" color='claimzTextBlackColor'>{rowData.contact_no}</Box></Text>
                                <Text mb='10px' fontSize='1.6rem' fontWeight='600' color='claimzTextBlueColor'>Mail ID - <Box as="span" color='claimzTextBlackColor'>{rowData.email}</Box></Text>
                                <Text mb='10px' fontSize='1.6rem' fontWeight='600' color='claimzTextBlueColor'>PAN - <Box as="span" color='claimzTextBlackColor'>{rowData.pan_no}</Box></Text>
                                <Text mb='10px' fontSize='1.6rem' fontWeight='600' color='claimzTextBlueColor'>Aadhar No - <Box as="span" color='claimzTextBlackColor'>{rowData.aadhar_no}</Box></Text>
                                <Text mb='10px' fontSize='1.6rem' fontWeight='600' color='claimzTextBlueColor'>Passport No - <Box as="span" color='claimzTextBlackColor'>{rowData.passport_no}</Box></Text>
                            </Box>
                            <Box background='var(--chakra-colors-claimzMainGeadientColor)' mb='15px' p='10px 10px 15px' color='white'>
                            ...
                            <Box>
                                <Box display='flex' justifyContent='space-between' mb='10px'>
                                    <Text fontWeight='600' color='#019148' fontSize='1.4rem'>{formattedStartDate}</Text>
                                    <Text fontWeight='600' color='#F21D1D' fontSize='1.4rem'>{formattedEndDate}</Text>
                                </Box>
    ...
    return (
        <CssWrapper >
            {loader ?
                <Box height='calc(100vh - 215px)' display='flex'
                    alignItems='center'
                    justifyContent='center'>
                ...
                <Box className="card">
                    <DataTable value={empList?.data} header={header} filters={filters} onFilter={(e) => setFilters(e.filters)} dataKey="id" >
                        ...
                        <Column header="Details" body={ActionTemplate} bodyStyle={{ textAlign: 'center' }} style={{ width: '14%' }} />
