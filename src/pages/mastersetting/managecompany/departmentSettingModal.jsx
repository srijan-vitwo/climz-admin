...
const DepartmentSettingModal = ({ rowData }) => {
    ...
    const addCostCenter = async (e) => {
        ...
        try {
            setIsLoading(true)
            const response = await fetch(`${process.env.REACT_APP_API_URL}/cost-center-post/${departmentId}`, {
                method: "POST",
                body: formData,
                headers: {
    ...
    return (
        <CssWrapper>
            <Button onClick={modalOnOpen} bg='none' _hover={{ bg: 'none' }}
                _active={{ bg: 'none' }}
                _focus={{ bg: 'none' }}><i className="fa-solid fa-eye fa-2x"></i></Button>
            <Modal isOpen={modalIsOpen} onClose={modalOnClose} isCentered>
                <ModalOverlay bg='rgba(0,0,0,0.2)' />
                <ModalContent minW='70%' h='70vh'>
                    <ModalHeader pt='50px' pb='15px' display='flex' justifyContent='space-between' alignItems='center'>
                        <Box
                            ...
                            <Text background='linear-gradient(180deg, #2770AE 0%, #01325B 100%)'
                                ...
                                fontWeight='700'
                                fontSize='28px'
                                lineHeight='36px'>Cost Center List</Text>
                        </Box >
                        <Button
                            display='flex'
                            alignItems='center'
                        ...
                        >
                            <Box as='span' mr='10px'> Add More Cost Center </Box> <i className="fa-sharp fa-solid fa-plus"></i>
                        </Button>
                    </ModalHeader>
                                    ...
                                    {inputs.map((input, index) => (
                                        <Box w='100%' display='flex' alignItems='center' key={index}>
                                            ...
                                            <Button mt='-10px' color='var(--chakra-colors-claimzTextBlueLightColor)' bg='none' _hover={{ bg: 'none' }}
                                                _active={{ bg: 'none' }}
                                                _focus={{ bg: 'none' }} onClick={() => handleRemoveInput(index)}><i className="fa-solid fa-trash fa-2x"></i></Button>
                                        </Box>
                                    ))}
                                    {flag && <Button
                                        disabled={isLoading}
