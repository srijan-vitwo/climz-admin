...
const AddDepartment = () => {
    ...
    const departmentAdd = async (e) => {
        ...
        try {
            setIsLoading(true)
            const response = await fetch(`${process.env.REACT_APP_API_URL}/department-post`, {
                method: "POST",
                body: formData,
                headers: {
                    Authorization: `Bearer ${token}`,
                    ...
                    <form onSubmit={departmentAdd}>
                        <Box display='flex' flexDirection='column' alignItems='end'>
                            <FormControl mb='10px'>
                                ...
                                <Input type='text' onChange={e => setDepartment(e.target.value)} required />
                            </FormControl>
                            <FormControl mb='10px'>
                                <FormLabel>Select Head of Department</FormLabel>
                                <Select placeholder='Select option' required onChange={(e) => setHod(e.target.value)}>
                            ...
                            <FormControl w='100%' alignItems='center'>
                                <FormLabel>Add Cost Center <Button w='20%' p='0px' width='10px' bg='none' _hover={{ bg: 'none' }}
                                    _active={{ bg: 'none' }}
