...
const AllpayrolllReportTable = ({ dataList }) => {
    ...
    dataList && Object.entries(dataList[0]["attendance"]).map((rowData, rowKey) => {
        attendanceHeaders.push({
            field: `${rowData[0]}`, header: `${rowData[0]}`
        });
    });

    let earningHeaders = [];
    dataList && Object.entries(dataList[0]["earning"]).map((rowData, rowKey) => {
        earningHeaders.push({
            field: `${rowData[0]}`, header: `${rowData[0]}`
        });
    });
    ...
    dataList && Object.entries(dataList[0]["deduction"]).map((rowData, rowKey) => {
    ...
    dataList?.map((rowData, rowKey) => {
        dataList[rowKey] = { ...rowData, ...rowData.attendance, ...rowData.earning, ...rowData.deduction };
                ...
                <Box display='flex' justifyContent='space-between'>
                    ...
                    <Button border='2px solid var(--chakra-colors-claimzBorderColor)' borderRadius='15px' height='45px' padding='0px 20px' mr='10px' type="button" icon="pi pi-file-excel" severity="success" background='linear-gradient(180deg, #2770AE 0%, #01325B 100%)'
                        ...
                        <Text background='linear-gradient(180deg, #2770AE 0%, #01325B 100%)'
                            backgroundClip='text'
                            fontSize='1.6rem' fontWeight='700'>
                            <Box as='span' mr='10px'>Export as Excel</Box><i className="fa-solid fa-file-excel"></i>
                        </Text>
                    </Button>
                </Box >
            </Box>
    ...
    return (
        <CssWrapper >
            <Box className="card">
                <DataTable value={dataList} header={header} filters={filters} onFilter={(e) => setFilters(e.filters)} dataKey="emp_id">
                    {cols.map((col, index) => (
                        <Column
                            key={index}
                            field={col.field}
