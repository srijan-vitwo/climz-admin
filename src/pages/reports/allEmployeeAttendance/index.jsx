...
const AllEmployeeAttendanceReportView = () => {
    ...
    useEffect(() => {
        const attendanceReportTable = async () => {
            try {
                setLoader(true)
                const response1 = await fetch(`${process.env.REACT_APP_API_URL}/all-employee-attendance/${date1stValue}/${date2ndValue}/${rows}`, {
                    method: "GET",
                    headers: {
    ...
    return (
        <Card>
            ...
            {loader ?
                ...
                :
                <AllEmployeeAttendanceReportTable dataList={listValue} rows={rows} setRows={setRows} first={first} setFirst={setFirst} />
            }
