...
`
const EmployeeAttendanceReportTable = ({ dataList }) => {

    const dt = useRef(null);
    const cols = [
        { field: 'date', header: 'Date' },
        { field: 'checkin', header: 'In Time' },
        { field: 'in_address', header: 'Checked At' },
        { field: 'checkout', header: 'Out Time' },
        { field: 'out_address', header: 'Checked Out At' },
    ...
    return (
        <CssWrapper >
            <Box className="card">
                <DataTable value={dataList} header={header} filters={filters} onFilter={(e) => setFilters(e.filters)} dataKey="emp_code" >
                    {cols.map((col, index) => (
                        <Column key={index} field={col.field} header={col.header} style={{ width: '14%' }} />
                    ))}
                </DataTable>
            </Box>
        </CssWrapper>
