...
import { Box, Heading, Table, Tbody, Td, Th, Thead, Tr, Spinner } from '@chakra-ui/react'

const UpcomingHolidays = ({ UpComingEvent, Loader }) => {

    return (
        <Box >
            <Heading mb='15px' color='claimzTextBlueColor'>Up Coming Events</Heading>
            {Loader ?
                <Box display='flex' alignItems='center' justifyContent='center' height='380px' >
                    <Spinner
                        thickness='4px'
                        speed='0.65s'
                        emptyColor='gray.200'
                        ...
                        <Tbody sx={{
                        ...
                        }}>
                            {
                                UpComingEvent?.length <= 0 ?
                                    <Tr><Td colSpan={4} textAlign='center'>No record found</Td></Tr>
                                    :
                                    <>
                                        {UpComingEvent?.map((data, index) => {
                                            ...
                                            targetDate.setMonth(parseInt(month) - 1); // Set the month (zero-based index)
